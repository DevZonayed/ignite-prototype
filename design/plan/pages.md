...
- ☐ Working router: nav item → shows that view, updates title + `#hash`.
...
- ☐ States represented in-app: loading skeleton, empty state, error/retry, offline banner.
...
- ☐ Make the Curriculum unit tree an actual **accordion** (units expand/collapse); tree lesson click loads that lesson in the editor.
- ☐ Wire row actions (Manage/Edit) to a slide-over/modal (at least a working modal shell).
...
- ☐ States: offline banner, sync states, empty (no lessons), toast on save.
